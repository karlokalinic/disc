...
public class FlowController : MonoBehaviour
{
    ...
    public void Initialize()
    {
        ...
        _initialized = true;
        if (_active != null && _active != this) { Destroy(gameObject); return; }
        _active = this;
        if (FindAnyObjectByType<EventSystem>() == null)
            new GameObject("EventSystem", typeof(EventSystem), typeof(StandaloneInputModule));
        RecoveredRuntimeServices.EnsureSingleEventSystem();
        LoadUISprites();
        Debug.Log("[FLOWUI] drawing loading screen; texture cache="+_uiTextures.Count);
        BuildCanvas();
        Show(Screen.Loading);
    ...
    void Archetype()
    {
        ...
        Header("CREATE YOUR DETECTIVE", "Choose a template or tune the faculties yourself.");
        ...
        for(int i=0;i<4;i++)
        {
            int idx=i; var x=140+i*430;
            var panel=Panel(new Vector2(x,-740),new Vector2(x+390,-260));
            Text(panel,names[i],38,new Color(0.88f,0.70f,0.32f),new Vector2(20,-70),new Vector2(-20,-20),TextAlignmentOptions.Center);
            string stat=$"INT {vals[i][0]}   PSY {vals[i][1]}   FYS {vals[i][2]}   MOT {vals[i][3]}";
            Text(panel,stat,24,Color.white,new Vector2(20,-130),new Vector2(-20,-90),TextAlignmentOptions.Center);
            Text(panel,"SIGNATURE: "+skills[sigs[i][0]][sigs[i][1]],22,new Color(0.86f,0.30f,0.22f),new Vector2(20,-190),new Vector2(-20,-145),TextAlignmentOptions.Center);
            SkillPortrait(panel, sigs[i][0], sigs[i][1], new Vector2(110,-335), new Vector2(-110,-205));
            Button(panel,"SELECT",28,new Vector2(55,-420),new Vector2(-55,-350),()=>{ ApplyArchetype(idx); Show(Screen.Abilities); });
        }
        Button(_root,"BACK",26,new Vector2(140,-1010),new Vector2(330,-950),()=>Show(Screen.Menu));
    }
